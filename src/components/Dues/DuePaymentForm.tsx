...
import { Save, X, Upload, DollarSign, Calendar, AlertCircle, CheckCircle } from 'lucide-react';
import { Due, DuePayment } from '../../types';

interface DuePaymentFormProps {
  due: Due;
  onSave: (payment: Omit<DuePayment, 'id' | 'dueId'>) => void;
  onCancel: () => void;
}

const DuePaymentForm: React.FC<DuePaymentFormProps> = ({ due, onSave, onCancel }) => {
  const [formData, setFormData] = useState<Omit<DuePayment, 'id' | 'dueId'>>({
    amount: 0,
  ...
  const validateForm = () => {
    ...
    } else if (formData.amount > due.remainingAmount) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // تأكيد السداد الكامل
    if (isFullPayment && formData.amount === due.remainingAmount) {
      if (!confirm('هل أنت متأكد من سداد المبلغ المتبقي بالكامل؟ سيتم إغلاق هذا المستحق نهائياً.')) {
        return;
      }
    }
  ...
  const handleFullPayment = () => {
    ...
    setFormData(prev => ({ ...prev, amount: due.remainingAmount }));
  };
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    سداد المبلغ المتبقي بالكامل ({due.remainingAmount.toLocaleString()} ريال)
                  </p>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  الحد الأقصى: {due.remainingAmount.toLocaleString()} ريال
                </p>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {formData.type === 'bank_transfer' ? 'رقم العملية *' : 'رقم السند'}
                </label>
                <input
                  type="text"
                  ...
                  placeholder={formData.type === 'bank_transfer' ? 'أدخل رقم العملية' : 'أدخل رقم السند (اختياري)'}
                  required={formData.type === 'bank_transfer'}
                />
