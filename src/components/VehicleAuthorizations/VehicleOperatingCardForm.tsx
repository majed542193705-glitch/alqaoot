...
import { X, Upload, FileText, Car, Calendar, CreditCard } from 'lucide-react';
import { VehicleOperatingCard, Vehicle } from '../../types';
import { useTheme } from '../../contexts/ThemeContext';

interface VehicleOperatingCardFormProps {
  operatingCard?: VehicleOperatingCard;
  onSave: (operatingCard: Omit<VehicleOperatingCard, 'id' | 'createdAt' | 'updatedAt'>) => void;
  onCancel: () => void;
}

const VehicleOperatingCardForm: React.FC<VehicleOperatingCardFormProps> = ({
  operatingCard,
  onSave,
  onCancel
}) => {
  ...
  const [formData, setFormData] = useState({
    vehicleId: operatingCard?.vehicleId || '',
    cardNumber: operatingCard?.cardNumber || '',
    startDate: operatingCard?.startDate || '',
    expiryDate: operatingCard?.expiryDate || '',
    attachments: operatingCard?.attachments || [],
    notes: operatingCard?.notes || ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const operatingCardData = {
      ...formData,
      attachments: [...formData.attachments, ...attachmentUrls]
    };

    onSave(operatingCardData);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {operatingCard 
                ? (language === 'ar' ? 'تعديل كرت التشغيل' : 'Edit Operating Card')
                : (language === 'ar' ? 'إضافة كرت تشغيل جديد' : 'Add New Operating Card')
              }
            </h2>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {language === 'ar' ? 'رقم كرت تشغيل المركبة' : 'Vehicle Operating Card Number'}
            </label>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              placeholder={language === 'ar' ? 'أدخل رقم كرت التشغيل' : 'Enter operating card number'}
              required
            />
          </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-4">
              <input
                ...
                className="hidden"
                id="operating-card-file-upload"
                accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
              />
              <label
                htmlFor="operating-card-file-upload"
...
export default VehicleOperatingCardForm;
